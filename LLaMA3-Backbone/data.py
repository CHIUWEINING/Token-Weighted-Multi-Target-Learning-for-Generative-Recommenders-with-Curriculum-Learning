...
class BaseDataset(Dataset):
    ...
    def get_prefix_allowed_tokens_fn(self, tokenizer):
            ...
            for index in self.indices.values():
                for i, token in enumerate(index):
                    ...
                    if token_id is None or token_id < 0 or token_id == tokenizer.unk_token_id:
                        ...
                        if len(ids) != 1:
                            raise ValueError(f"Token '{token}' is not a single tokenizer id: {ids}")
                        token_id = ids[0]
        ...
        for sep_text in ("\n\n### Response:", "### Response:", "Response:"):
            ...
            if sep_ids:
                sep_candidates.append(sep_ids)
        fallback_tokens = list(self.allowed_tokens.get(0, {tokenizer.eos_token_id}))
        ...
        def prefix_allowed_tokens_fn(batch_id, sentence):
            ...
            for sep in sep_candidates:
                sep_reversed = sep[::-1]
                sep_len = len(sep_reversed)
                for i in range(len(reversed_sent)):
                    if reversed_sent[i:i + sep_len] == sep_reversed:
                        return list(self.allowed_tokens.get(i, {tokenizer.eos_token_id}))
...
class SeqRecDataset(BaseDataset):
    ...
    def _load_data(self):
        inter_file = self.dataset + (f".{self.args.eval_mode}_" if self.args.eval_mode != "" else ".") + "inter.json"
        with open(os.path.join(self.data_path, inter_file), "r") as f:
    ...
    def _process_train_data(self):
        ...
        for uid in self.remapped_inters:
            ...
            if self.args.core7:
                if len(self.remapped_inters[uid]) < 7:
                    continue
            ...
            for i in range(1, len(items)):
                one_data = {}
                one_data["user"] = uid
                one_data["item_id"] = int(self.inters[uid][i])
                one_data["other_items"] = None
    ...
    def _process_valid_data(self):
        ...
        for uid in self.remapped_inters:
            ...
            one_data = {}
            one_data["user"] = uid
            one_data["item_id"] = int(self.inters[uid][-2])
            one_data["other_items"] = None
            ...
            if self.args.core7:
                if len(self.remapped_inters[uid]) < 7:
                    continue
                items = self.remapped_inters[uid][:-2]
                one_data["item_id"] = int(self.inters[uid][:-2][-2])
    ...
    def _process_test_data(self):
        inter_data = []
        for uid in self.remapped_inters:
            if self.args.core7 and len(self.remapped_inters[uid]) < 7:
                continue
            ...
            one_data = {}
            one_data["user"] = uid
            one_data["item_id"] = int(self.inters[uid][-1])
            one_data["other_items"] = None
            one_data["item"] = items[-1]

            if self.args.core7:
                if self.remapped_inters[uid][-2] in items[:-1] or self.remapped_inters[uid][-1] in items[:-1]:
                    continue
                one_data["other_items"] = [self.remapped_inters[uid][-2], self.remapped_inters[uid][-1]]
        ...
        if self.sample_valid:
            ...
            for i in range(len(self.inter_data)):
                ...
                for prompt_id in prompt_ids:
                    ...
                    input_text, output_text, _ = self._get_text_data(d, prompt)
                    self.valid_text_data.append(
                        {
                            "input_ids": input_text,
                            "labels": output_text,
                            "item_ids": d.get("item_id", -1),
                            "other_labels": d.get("other_items"),
                            "user": d.get("user"),
                        }
                    )
        else:
    ...
    def _get_text_data(self, data, prompt):
        ...
        cf_data = dict(data)
        cf_data["inters"] = data.get("cf_inters", "")
        cf_instruction = prompt["instruction"].format(**cf_data)

        input_text = sft_prompt.format(instruction=instruction, response="")
        cf_input = sft_prompt.format(instruction=cf_instruction, response="")
        output_text = sft_prompt.format(instruction=instruction, response=response)

        if self.mode == "test":
            return input_text, response, cf_input
        return input_text, output_text, cf_input
    ...
    def __getitem__(self, index):
        ...
        prompt = self.prompts[prompt_id]
        input_text, output_text, cf_input = self._get_text_data(d, prompt)

        return {
            "input_ids": input_text,
            "labels": output_text,
            "cf_input_ids": cf_input,
            "cf_labels": output_text,
            "item_ids": d.get("item_id", -1),
            "other_labels": d.get("other_items"),
            "user": d.get("user"),
        }

    def process_raw_labels(self):
        processed_user = []
        processed_label = []
        for user, inters in self.inters.items():
            processed_user.append(user)
            processed_label.append(inters[-1])
        return processed_user, processed_label
