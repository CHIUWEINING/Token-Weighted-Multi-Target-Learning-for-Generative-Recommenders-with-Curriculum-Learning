...
import argparse
import os
from utils import random_assign_to_avoid_collision
...
def parse_args():
    ...
    parser.add_argument("--data_path", type=str, default="/tmp/wmir_final_dataset/", help='data path')
    ...
    parser.add_argument("--alpha_", type=float, default = None, help="for ema")
...
args_setting.data_path = args_setting.data_path + args_setting.dataset + "/"+ args_setting.dataset + (".emb-llama-td.npy" if args_setting.alpha_==None else f".emb-llama-td-{args_setting.alpha_}.npy")#sentence-t5-xl
data = EmbDataset(args_setting.data_path, args_setting)
...
all_indices_dict = random_assign_to_avoid_collision(all_indices_dict, num_emb_list=args.num_emb_list)
